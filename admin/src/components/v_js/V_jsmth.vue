<template>
    <div class="form-container">
      <el-form
        ...
        :model="formData">
        <div class="Form">
          <el-tag type="primary" style="margin-bottom: 10px">方法录入模块</el-tag>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        name: [{ required: true, message: '请输入方法名', trigger: 'blur' }],
        desc: [{ required: true, message: '请输入方法描述', trigger: 'blur' }],
  ...
  methods: {
    submitForm (value) {
      this.$refs[value].validate((valid) => {
        if (valid) {
          this.$axios.post('/js/mth/insertOne', value)
            .then(data => {
              // 添加成功
              this.$message({
